...
var project_list = function(db, projectName, options) {
    ...
    if (projectName === undefined) {
        return projectsCol.find({}).toArray().catch(err => Boom.wrap(err));
    } else {
    ...
    return files_list(db, projectName, fileName).then((results) => {
        ...
        if (lineId !== undefined) {
            return linesCol.find({
                _id: fileId | lineId
            }).toArray().catch(err => Boom.wrap(err));
        }
        ...
        return findPromise.catch(err => Boom.wrap(err));
    });
};
...
var insert_trans = async(function*(db, projectName, fileName, lineId, text, userId) {
    ...
    let result = yield transCol.updateOne({_id: _lineId}, {
        $push: {
            'commits': {
                text: text,
                userId: userId,
